...
$routes = [
    ...
    ['GET',  '#^rider/bookings/(\d+)$#',                fn($id) => api_rider_booking_get($pdo, (int) $id)],
    ...
    ['GET',  '#^rider/training$#',                      fn() => api_rider_training_get($pdo)],
    ['POST', '#^rider/training/complete$#',             fn() => api_rider_training_complete($pdo)],
    ...
    ['POST', '#^admin/riders/(\d+)/balance/otp$#',      fn($id) => api_admin_rider_balance_otp_request($pdo, (int) $id)],
    ['POST', '#^admin/riders/(\d+)/balance/verify$#',   fn($id) => api_admin_rider_balance_verify($pdo, (int) $id)],
...
function api_list_bookings(PDO $pdo): void {
    ...
    // data must be the bare array (not wrapped) - the Android client's ApiResponse<List<Booking>>
    // deserializes `data` directly as a JSON array; a {"bookings": [...]} wrapper fails to parse.
    api_ok($bookings, ['cursor' => $nextCursor]);
}
