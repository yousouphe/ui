...
function compute_sender_booking_flags(?array $booking): array
{
    if (!$booking) {
        return [
            'canTrack' => false,
    ...
    $canTrack = !empty($booking['selected_rider_user_id']);
    $needsRider = empty($booking['selected_rider_user_id']) && in_array($bookingStatus, ['submitted'], true);
    $canPay = $bookingStatus === 'delivered' && $paymentStatus !== 'paid';

    $canCancel = !empty($booking['selected_rider_user_id'])
        && in_array($bookingStatus, ['matched', 'accepted', 'arrived_at_pickup', 'package_received', 'in_transit'], true)
        && $paymentStatus !== 'paid';

    $canRebook = $bookingStatus === 'cancelled' && $paymentStatus !== 'paid';

    $canIssueItem = $bookingStatus === 'arrived_at_pickup'
        && !empty($booking['selected_rider_user_id'])
        && (int)($booking['sender_handover_confirmed'] ?? 0) === 0;

    $canChat = (int)($booking['selected_rider_user_id'] ?? 0) > 0;
...
$flags = compute_sender_booking_flags($selectedBooking);
$canTrack = $flags['canTrack'];
$needsRider = $flags['needsRider'];
$canPay = $flags['canPay'];
$canCancel = $flags['canCancel'];
$canRebook = $flags['canRebook'];
$canIssueItem = $flags['canIssueItem'];
...
$selectedRiderPhone = $selectedBooking['rider_phone'] ?? '';
$selectedRiderName = $selectedBooking['rider_name'] ?? 'Rider';

// ---------------- AJAX SNAPSHOT ----------------
if (isset($_GET['ajax']) && $_GET['ajax'] === 'snapshot') {
    $snapshotBookingId = isset($_GET['booking_id']) ? (int)$_GET['booking_id'] : 0;

    if ($snapshotBookingId <= 0) {
        respond_json(['success' => false, 'message' => 'Invalid booking id.'], 422);
    }
    ...
    $stmt = $pdo->prepare("
        ...
        LIMIT 1
    ");
    $stmt->execute([$snapshotBookingId, $user['id']]);
    $snapBooking = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$snapBooking) {
        respond_json(['success' => false, 'message' => 'Booking not found.'], 404);
    }

    $snapFlags = compute_sender_booking_flags($snapBooking);
    $snapDistanceKm = null;
    if (
        $snapBooking['pickup_latitude'] !== null &&
        $snapBooking['pickup_longitude'] !== null &&
        $snapBooking['delivery_latitude'] !== null &&
        $snapBooking['delivery_longitude'] !== null
    ) {
        $snapDistanceKm = haversine_distance(
            (float)$snapBooking['pickup_latitude'],
            (float)$snapBooking['pickup_longitude'],
            (float)$snapBooking['delivery_latitude'],
            (float)$snapBooking['delivery_longitude']
        );
    }

    $counts = [
        'active' => count($activeBookings),
        'unpaid' => count($unpaidBookings),
    ...
    respond_json([
        ...
        'counts' => $counts,
        'booking' => [
            'id' => (int)$snapBooking['id'],
            'booking_code' => (string)($snapBooking['booking_code'] ?? ''),
            'booking_status' => (string)($snapBooking['booking_status'] ?? ''),
            'payment_status' => (string)($snapBooking['payment_status'] ?? 'unpaid'),
            'rider_name' => (string)($snapBooking['rider_name'] ?? 'Not assigned yet'),
            'rider_phone' => (string)($snapBooking['rider_phone'] ?? ''),
            'selected_rider_user_id' => (int)($snapBooking['selected_rider_user_id'] ?? 0),
            'sender_handover_confirmed' => (int)($snapBooking['sender_handover_confirmed'] ?? 0),
            'cancellation_reason' => (string)($snapBooking['cancellation_reason'] ?? ''),
            'delivery_proof_image' => (string)($snapBooking['delivery_proof_image'] ?? ''),
            'agreed_cost' => (float)($snapBooking['agreed_cost'] ?? 0),
            'distance_km' => $snapDistanceKm,
            'pickup_address' => (string)($snapBooking['pickup_address'] ?? ''),
            'delivery_address' => (string)($snapBooking['delivery_address'] ?? ''),
            'recipient_name' => (string)($snapBooking['recipient_name'] ?? ''),
            'recipient_phone' => (string)($snapBooking['recipient_phone'] ?? ''),
            'tab' => $snapFlags['selectedBookingTab'],
        ],
        'flags' => [
            'canTrack' => $snapFlags['canTrack'],
            'needsRider' => $snapFlags['needsRider'],
            'canPay' => $snapFlags['canPay'],
            'canCancel' => $snapFlags['canCancel'],
            'canRebook' => $snapFlags['canRebook'],
            'canIssueItem' => $snapFlags['canIssueItem'],
            'canChat' => $snapFlags['canChat'],
        ],
    ]);
}
?>
...
<head>
    ...
    <style>
        ...
        .chat-panel{position:fixed;right:20px;bottom:90px;width:420px;max-width:calc(100vw - 24px);height:600px;max-height:78vh;z-index:100000;border-radius:1.25rem;background:rgba(8,17,33,.72);backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px);border:1px solid rgba(255,255,255,.10);box-shadow:0 20px 40px rgba(0,0,0,.35);display:none;overflow:hidden}
        .chat-header{padding:14px 16px;border-bottom:1px solid rgba(255,255,255,.08);display:flex;justify-content:space-between;align-items:center;gap:10px}
        .chat-messages{height:380px;overflow-y:auto;padding:14px;display:flex;flex-direction:column;gap:10px}
        .chat-bubble{max-width:82%;padding:10px 12px;border-radius:14px;font-size:.92rem;line-height:1.35;word-wrap:break-word}
        ...
        .chat-toolbar{display:flex;gap:8px;align-items:center;margin-bottom:8px}
        .voice-status{font-size:.75rem;color:#9fb0d6}
        .voice-audio{display:block;max-width:220px;margin-top:8px}
        .call-btn-mini{min-width:42px}
        @media (max-width:576px){
            .sticky-chat-btn{right:14px;bottom:14px}
            .chat-panel{right:12px;left:12px;width:auto;bottom:84px;height:72vh}
        }
    </style>
</head>
<body>
...
<div class="container py-5">
    <div
        ...
        data-rider-phone="<?= e((string)$selectedRiderPhone) ?>"
        data-rider-name="<?= e((string)$selectedRiderName) ?>"
        data-snapshot-url="<?= e(url_path('bookings/index.php?ajax=snapshot')) ?>"
    >
                ...
                <div class="row g-4">
                    ...
                    <div class="col-lg-8">
                        <?php if ($selectedBooking && $selectedBookingIsActive): ?>
                            ...
                            <div class="row g-4 mb-4">
                                ...
                                <div class="col-lg-4">
                                    <div class="cardx p-4 h-100">
                                        ...
                                        <div class="small text-soft mb-2"><strong class="text-white">Recipient:</strong> <span id="recipient_info_text"><?= e($selectedBooking['recipient_name']) ?> · <?= e($selectedBooking['recipient_phone']) ?></span></div>
                                        <div class="small text-soft mb-2"><strong class="text-white">Pickup:</strong> <span id="pickup_info_text"><?= e($selectedBooking['pickup_address']) ?></span></div>
                                        <div class="small text-soft mb-2"><strong class="text-white">Delivery:</strong> <span id="delivery_info_text"><?= e($selectedBooking['delivery_address']) ?></span></div>
                                        <div class="small text-soft mb-2"><strong class="text-white">Distance:</strong> <span id="distance_info_text"><?= $selectedDistanceKm !== null ? number_format($selectedDistanceKm, 2) . ' km' : '--' ?></span></div>
                                        <div class="small text-soft mb-2"><strong class="text-white">Rider:</strong> <span data-role="rider-name"><?= e((string) ($selectedBooking['rider_name'] ?? 'Not assigned yet')) ?></span></div>
                                        <div class="small text-soft mb-2"><strong class="text-white">Rider Phone:</strong> <span data-role="rider-phone"><?= e((string) ($selectedBooking['rider_phone'] ?? '--')) ?></span></div>

                                        <div class="mt-2" id="booking-call-wrap" style="<?= !empty($selectedBooking['rider_phone']) ? '' : 'display:none;' ?>">
                                            <a id="booking-call-btn" href="<?= !empty($selectedBooking['rider_phone']) ? 'tel:' . e($selectedBooking['rider_phone']) : '#' ?>" class="btn btn-outline-info w-100">
                                                <i class="fa-solid fa-phone me-2"></i>Call Rider
                                            </a>
                                        </div>

                                        <div class="mt-3 <?= !empty($selectedBooking['delivery_proof_image']) ? '' : 'd-none' ?>" id="delivery-proof-wrap">
                                            <div class="small text-white fw-bold mb-2">Proof of Delivery</div>
                                            <img id="delivery-proof-image" src="<?= !empty($selectedBooking['delivery_proof_image']) ? e(url_path($selectedBooking['delivery_proof_image'])) : '' ?>" class="img-fluid rounded" alt="Proof">
                                        </div>

                                        <div class="mt-4 action-stack">
                                            <div id="pay-btn-wrap" style="<?= $canPay ? '' : 'display:none;' ?>">
                                                <button class="btn btn-success w-100" type="button" id="pay-now-btn" data-booking-id="<?= (int) $selectedBooking['id'] ?>">
                                                    Pay ₦<span id="pay_amount_text"><?= number_format((float) $selectedBooking['agreed_cost'], 2) ?></span>
                                                </button>
                                            </div>

                                            <div id="issue-item-wrap" style="<?= $canIssueItem ? '' : 'display:none;' ?>">
                                            ...
                                            <div id="issue-item-done-wrap" style="<?= ($selectedBooking && (int)($selectedBooking['sender_handover_confirmed'] ?? 0) === 1) ? '' : 'display:none;' ?>">
                                                <div class="alert alert-info mb-0">
                                                    <i class="fa-solid fa-circle-check me-2"></i>Item already issued to rider.
                                                </div>
                                            </div>

                                            <div id="cancel-btn-wrap" style="<?= $canCancel ? '' : 'display:none;' ?>">
                                            ...
                                            <div id="rebook-btn-wrap" style="<?= $canRebook ? '' : 'display:none;' ?>">
                                        ...
                                        <div id="cancel-reason-wrap" class="cancel-reason-box <?= $cancellationReason !== '' ? '' : 'd-none' ?>">
                                            <div class="fw-bold mb-1">Cancellation Reason</div>
                                            <div class="small" id="cancel-reason-text"><?= e($cancellationReason) ?></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div id="nearby-riders-section" style="<?= $needsRider ? '' : 'display:none;' ?>">
                                <div class="cardx p-4 mb-4">
                            ...
                            <div class="row g-4 mb-4">
                                <div class="col-12">
                                    <div class="cardx p-4">
                                        <h3 class="h5 mb-3">Booking Info</h3>
                                        <div class="row g-3">
                                            <div class="col-md-6 small text-soft"><strong class="text-white">Recipient:</strong> <span id="recipient_info_text"><?= e($selectedBooking['recipient_name']) ?> · <?= e($selectedBooking['recipient_phone']) ?></span></div>
                                            <div class="col-md-6 small text-soft"><strong class="text-white">Distance:</strong> <span id="distance_info_text"><?= $selectedDistanceKm !== null ? number_format($selectedDistanceKm, 2) . ' km' : '--' ?></span></div>
                                            <div class="col-md-6 small text-soft"><strong class="text-white">Pickup:</strong> <span id="pickup_info_text"><?= e($selectedBooking['pickup_address']) ?></span></div>
                                            <div class="col-md-6 small text-soft"><strong class="text-white">Delivery:</strong> <span id="delivery_info_text"><?= e($selectedBooking['delivery_address']) ?></span></div>
                                            <div class="col-md-6 small text-soft"><strong class="text-white">Rider:</strong> <span data-role="rider-name"><?= e((string) ($selectedBooking['rider_name'] ?? 'Not assigned yet')) ?></span></div>
                                            <div class="col-md-6 small text-soft"><strong class="text-white">Rider Phone:</strong> <span data-role="rider-phone"><?= e((string) ($selectedBooking['rider_phone'] ?? '--')) ?></span></div>
                                        </div>
        ...
        <div class="chat-panel" id="chat-panel">
            <div class="chat-header">
                <div>
                    ...
                    <div class="small text-soft" id="chat-rider-name"><?= e((string)($selectedBooking['rider_name'] ?? 'Rider')) ?></div>
                </div>
                <div class="d-flex align-items-center gap-2">
                    <a href="<?= !empty($selectedBooking['rider_phone']) ? 'tel:' . e($selectedBooking['rider_phone']) : '#' ?>" id="chat-call-btn" class="btn btn-sm btn-outline-info call-btn-mini <?= !empty($selectedBooking['rider_phone']) ? '' : 'disabled' ?>">
            ...
            <div class="chat-footer">
                <form id="chat-form">
                    ...
                    <div class="chat-toolbar">
                        <button type="button" id="voice-note-btn" class="btn btn-outline-warning">
                            <i class="fa-solid fa-microphone"></i>
                        </button>
                        <span class="voice-status" id="voice-status">Tap microphone to record voice note</span>
                    </div>
...
function showWorkspaceAlert(root, message, type = 'success') {
...
function cleanupWorkspace() {
    if (workspaceState.ridersInterval) clearInterval(workspaceState.ridersInterval);
    if (workspaceState.trackingInterval) clearInterval(workspaceState.trackingInterval);
    if (workspaceState.chatInterval) clearInterval(workspaceState.chatInterval);
    if (workspaceState.snapshotInterval) clearInterval(workspaceState.snapshotInterval);
    if (workspaceState.recordingTimer) clearInterval(workspaceState.recordingTimer);

    workspaceState.ridersInterval = null;
    workspaceState.trackingInterval = null;
    workspaceState.chatInterval = null;
    workspaceState.snapshotInterval = null;
    workspaceState.recordingTimer = null;
    workspaceState.recordingSeconds = 0;

    if (workspaceState.mediaRecorder && workspaceState.isRecording) {
        try { workspaceState.mediaRecorder.stop(); } catch (e) {}
    }
    workspaceState.mediaRecorder = null;
    workspaceState.mediaChunks = [];
    workspaceState.isRecording = false;
...
async function refreshSenderSnapshot(root) {
    if (!root) return;

    const selectedBookingId = parseInt(root.dataset.selectedBookingId || '0', 10);
    const snapshotBaseUrl = root.dataset.snapshotUrl || '';
    if (!selectedBookingId || !snapshotBaseUrl) return;

    try {
        const url = `${snapshotBaseUrl}${snapshotBaseUrl.includes('?') ? '&' : '?'}booking_id=${encodeURIComponent(selectedBookingId)}`;
        const response = await fetch(url, {
            headers: {
        ...
        if (!response.ok || !result.success || !result.booking) return;

        const booking = result.booking;
        const flags = result.flags || {};
        const currentTab = root.dataset.selectedBookingTab || 'active-orders';

        if ((booking.tab || currentTab) !== currentTab) {
            ajaxLoadWorkspace(`<?= e(url_path('bookings/index.php')) ?>?booking_id=${selectedBookingId}`, false);
            return;
        }

        root.dataset.selectedBookingStatus = booking.booking_status || '';
        root.dataset.selectedHasRider = booking.selected_rider_user_id ? '1' : '0';
        root.dataset.needsRider = flags.needsRider ? '1' : '0';
        root.dataset.canTrack = flags.canTrack ? '1' : '0';
        root.dataset.canPay = flags.canPay ? '1' : '0';
        root.dataset.canChat = flags.canChat ? '1' : '0';
        root.dataset.riderPhone = booking.rider_phone || '';
        root.dataset.riderName = booking.rider_name || '';

        root.querySelectorAll('#booking_status_text').forEach(el => el.textContent = booking.booking_status || '');
        root.querySelectorAll('#payment_status_text').forEach(el => el.textContent = booking.payment_status || '');
        root.querySelectorAll('[data-role="rider-name"]').forEach(el => el.textContent = booking.rider_name || 'Not assigned yet');
        root.querySelectorAll('[data-role="rider-phone"]').forEach(el => el.textContent = booking.rider_phone || '--');
        root.querySelectorAll('#pickup_info_text').forEach(el => el.textContent = booking.pickup_address || '');
        root.querySelectorAll('#delivery_info_text').forEach(el => el.textContent = booking.delivery_address || '');
        root.querySelectorAll('#recipient_info_text').forEach(el => el.textContent = `${booking.recipient_name || ''}${booking.recipient_phone ? ' · ' + booking.recipient_phone : ''}`);
        root.querySelectorAll('#distance_info_text').forEach(el => {
            el.textContent = (booking.distance_km !== null && booking.distance_km !== undefined)
                ? `${Number(booking.distance_km).toFixed(2)} km`
                : '--';
        });

        const payAmountText = root.querySelector('#pay_amount_text');
        if (payAmountText) payAmountText.textContent = Number(booking.agreed_cost || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });

        const callWraps = root.querySelectorAll('#booking-call-wrap');
        const callBtns = root.querySelectorAll('#booking-call-btn');
        callWraps.forEach(el => el.style.display = booking.rider_phone ? '' : 'none');
        callBtns.forEach(el => el.setAttribute('href', booking.rider_phone ? `tel:${booking.rider_phone}` : '#'));

        const chatCallBtn = root.querySelector('#chat-call-btn');
        if (chatCallBtn) {
            chatCallBtn.setAttribute('href', booking.rider_phone ? `tel:${booking.rider_phone}` : '#');
            chatCallBtn.classList.toggle('disabled', !booking.rider_phone);
        }

        const chatRiderName = root.querySelector('#chat-rider-name');
        if (chatRiderName) chatRiderName.textContent = booking.rider_name || 'Rider';

        const proofWrap = root.querySelector('#delivery-proof-wrap');
        const proofImg = root.querySelector('#delivery-proof-image');
        if (proofWrap && proofImg) {
            if (booking.delivery_proof_image) {
                proofWrap.classList.remove('d-none');
                proofImg.src = booking.delivery_proof_image.startsWith('http')
                    ? booking.delivery_proof_image
                    : `<?= e(rtrim(base_url(), '/')) ?>/` + booking.delivery_proof_image.replace(/^\/+/, '');
            } else {
        ...
        const cancelReasonWrap = root.querySelector('#cancel-reason-wrap');
        const cancelReasonText = root.querySelector('#cancel-reason-text');
        if (cancelReasonWrap && cancelReasonText) {
            if (booking.cancellation_reason) {
                cancelReasonWrap.classList.remove('d-none');
                cancelReasonText.textContent = booking.cancellation_reason;
            } else {
                cancelReasonWrap.classList.add('d-none');
                cancelReasonText.textContent = '';
            }
        }

        const payWrap = root.querySelector('#pay-btn-wrap');
        if (payWrap) payWrap.style.display = flags.canPay ? '' : 'none';

        const issueWrap = root.querySelector('#issue-item-wrap');
        if (issueWrap) issueWrap.style.display = flags.canIssueItem ? '' : 'none';

        const issueDoneWrap = root.querySelector('#issue-item-done-wrap');
        if (issueDoneWrap) issueDoneWrap.style.display = (parseInt(booking.sender_handover_confirmed || 0, 10) === 1) ? '' : 'none';

        const cancelWrap = root.querySelector('#cancel-btn-wrap');
        if (cancelWrap) cancelWrap.style.display = flags.canCancel ? '' : 'none';

        const rebookWrap = root.querySelector('#rebook-btn-wrap');
        if (rebookWrap) rebookWrap.style.display = flags.canRebook ? '' : 'none';

        const riderSection = root.querySelector('#nearby-riders-section');
        if (riderSection) riderSection.style.display = flags.needsRider ? '' : 'none';

        if (!flags.needsRider && workspaceState.ridersInterval) {
            clearInterval(workspaceState.ridersInterval);
            workspaceState.ridersInterval = null;
        }

        if (flags.canTrack && !workspaceState.trackingInterval) {
            ajaxLoadWorkspace(`<?= e(url_path('bookings/index.php')) ?>?booking_id=${selectedBookingId}`, false);
            return;
        }
    } catch (err) {
        console.error('Sender snapshot refresh failed:', err);
    }
}
...
function initSenderWorkspace() {
    ...
    root.querySelectorAll('#hubTabs button[data-bs-toggle="tab"]').forEach(btn => {
        btn.addEventListener('shown.bs.tab', function () {
            ...
            setTimeout(() => {
                if (workspaceState.detailMap) workspaceState.detailMap.invalidateSize();
            }, 200);
        });
    });
    ...
    if (
        ...
        if (shouldSearchRiders) {
            ...
            async function updateRiders() {
                const liveStatus = root.dataset.selectedBookingStatus || selectedBookingStatus;
                const liveHasRider = root.dataset.selectedHasRider === '1';

                if (!shouldSearchRiders || liveHasRider || ['matched', 'accepted', 'picked_up', 'in_transit', 'delivered', 'cancelled', 'arrived_at_pickup', 'package_received'].includes(String(liveStatus))) {
                    stopRiderSearch();
                    return;
                }
                ...
                try {
                    ...
                    riders.forEach(rider => {
                        ...
                        if (!workspaceState.knownRiderIds.has(rider.id) && workspaceState.knownRiderIds.size > 0) newRiderFound = true;
                        ...
                        form.addEventListener('submit', async function (event) {
                            ...
                            try {
                                ...
                                if (!response.ok || !result.success) {
                                    throw new Error(result.error || result.debug || result.message || 'Failed to send request.');
                                }

                                showWorkspaceAlert(root, result.message || 'Rider request sent successfully.', 'success');
                                ajaxLoadWorkspace(`<?= e(url_path('bookings/index.php')) ?>?booking_id=${selectedBookingId}`, false);
                            } catch (err) {
                                console.error('send_request error:', err);
                                btn.disabled = false;
                                btn.innerHTML = originalHtml;
                                showWorkspaceAlert(root, String(err.message || 'Failed to send request.'), 'danger');
                            }
                        });
                    });
        ...
        if (canTrack) {
            async function pollTracking() {
                try {
                    ...
                    root.dataset.selectedBookingStatus = d.booking_status || root.dataset.selectedBookingStatus;
    ...
    if (selectedBookingId > 0) {
        refreshSenderSnapshot(root);
        workspaceState.snapshotInterval = setInterval(() => refreshSenderSnapshot(document.getElementById('sender-workspace-content')), 5000);
    }

    root.querySelectorAll('#pay-now-btn').forEach(payNowBtn => {
        payNowBtn.addEventListener('click', async function () {
            ...
            try {
                ...
                btn.disabled = false;
                btn.innerHTML = originalHtml;
                showWorkspaceAlert(root, String(err.message || 'Payment initialization failed.'), 'danger');
            }
        });
    });
    ...
    confirmCancelBookingBtn?.addEventListener('click', async function () {
        ...
        try {
            ...
            bootstrap.Modal.getInstance(root.querySelector('#cancelBookingModal'))?.hide();
            showWorkspaceAlert(root, result.message || 'Booking cancelled successfully.', 'success');
            ajaxLoadWorkspace(`<?= e(url_path('bookings/index.php')) ?>?booking_id=${encodeURIComponent(bookingId)}`, false);
        } catch (err) {
            btn.disabled = false;
            btn.innerHTML = originalHtml;
            showWorkspaceAlert(root, err.message || 'Unable to cancel booking.', 'danger');
        }
    });

    root.querySelectorAll('#rebook-rider-btn').forEach(rebookRiderBtn => {
        rebookRiderBtn.addEventListener('click', async function () {
            ...
            try {
                ...
                showWorkspaceAlert(root, result.message || 'Searching for another rider.', 'success');
                ajaxLoadWorkspace('<?= e(url_path('bookings/index.php')) ?>?booking_id=' + encodeURIComponent(bookingId), true);
            } catch (err) {
                btn.disabled = false;
                btn.innerHTML = originalHtml;
                showWorkspaceAlert(root, err.message || 'Unable to book another rider.', 'danger');
            }
        });
    });
    ...
    confirmIssueItemBtn?.addEventListener('click', async function () {
        ...
        try {
            ...
            bootstrap.Modal.getInstance(root.querySelector('#issueItemModal'))?.hide();
            showWorkspaceAlert(root, result.message || 'Item issued to rider successfully.', 'success');
            ajaxLoadWorkspace(`<?= e(url_path('bookings/index.php')) ?>?booking_id=${encodeURIComponent(bookingId)}`, false);
        } catch (err) {
            btn.disabled = false;
            btn.innerHTML = originalHtml;
            showWorkspaceAlert(root, err.message || 'Unable to issue item to rider.', 'danger');
        }
    });
    ...
    if (canChat) {
        ...
        const voiceNoteBtn = root.querySelector('#voice-note-btn');
        ...
        function resolveVoiceUrl(msg) {
            return msg.voice_note_url || msg.audio_url || msg.attachment_url || msg.file_url || '';
        }
        ...
        function renderChatMessages(messages) {
            ...
            chatMessages.innerHTML = messages.map(msg => {
                const voiceUrl = resolveVoiceUrl(msg);
                const body = voiceUrl
                    ? `<div><i class="fa-solid fa-microphone me-2"></i>Voice note<audio class="voice-audio" controls preload="none" src="${escapeHtml(voiceUrl)}"></audio></div>`
        ...
        async function uploadVoiceNote(blob) {
            ...
            const result = await response.json();
            if (!response.ok || !result.success) {
                throw new Error(result.message || 'Unable to send voice note.');
            }
        }
        ...
        function resetVoiceUI() {
            workspaceState.isRecording = false;
            workspaceState.recordingSeconds = 0;
            if (workspaceState.recordingTimer) {
                clearInterval(workspaceState.recordingTimer);
                workspaceState.recordingTimer = null;
            }
            if (voiceStatus) voiceStatus.textContent = 'Tap microphone to record voice note';
            if (voiceNoteBtn) {
                voiceNoteBtn.classList.remove('btn-danger');
                voiceNoteBtn.classList.add('btn-outline-warning');
                voiceNoteBtn.innerHTML = '<i class="fa-solid fa-microphone"></i>';
            }
        }
        ...
        async function startVoiceRecording() {
            ...
            workspaceState.mediaChunks = [];
            workspaceState.mediaRecorder = new MediaRecorder(stream);

            workspaceState.mediaRecorder.ondataavailable = function (e) {
                if (e.data && e.data.size > 0) workspaceState.mediaChunks.push(e.data);
            };

            workspaceState.mediaRecorder.onstop = async function () {
                try {
                    const blob = new Blob(workspaceState.mediaChunks, { type: 'audio/webm' });
                    if (!blob.size) {
                        resetVoiceUI();
                        return;
                    }
                    if (voiceStatus) voiceStatus.textContent = 'Uploading voice note...';
                    await uploadVoiceNote(blob);
                    resetVoiceUI();
                    await fetchChatMessages();
                } catch (err) {
                    resetVoiceUI();
                    showWorkspaceAlert(root, err.message || 'Unable to send voice note.', 'danger');
                } finally {
                    try {
            ...
            workspaceState.mediaRecorder.start();
            workspaceState.isRecording = true;
            workspaceState.recordingSeconds = 0;

            if (voiceStatus) voiceStatus.textContent = 'Recording... 00:00';
            if (voiceNoteBtn) {
                voiceNoteBtn.classList.remove('btn-outline-warning');
                voiceNoteBtn.classList.add('btn-danger');
                voiceNoteBtn.innerHTML = '<i class="fa-solid fa-stop"></i>';
            }

            workspaceState.recordingTimer = setInterval(() => {
                workspaceState.recordingSeconds++;
                const mm = String(Math.floor(workspaceState.recordingSeconds / 60)).padStart(2, '0');
                const ss = String(workspaceState.recordingSeconds % 60).padStart(2, '0');
                if (voiceStatus) voiceStatus.textContent = `Recording... ${mm}:${ss}`;
            }, 1000);
        }

        async function toggleVoiceRecording() {
            try {
                if (!workspaceState.isRecording) {
                    await startVoiceRecording();
                } else if (workspaceState.mediaRecorder) {
                    workspaceState.mediaRecorder.stop();
                }
            } catch (err) {
                resetVoiceUI();
                showWorkspaceAlert(root, err.message || 'Unable to record voice note.', 'danger');
            }
        }
        ...
        closeChatBtn?.addEventListener('click', function () {
            ...
            if (workspaceState.isRecording && workspaceState.mediaRecorder) {
                try { workspaceState.mediaRecorder.stop(); } catch (e) {}
            } else {
                resetVoiceUI();
            }
        ...
        chatForm?.addEventListener('submit', async function (e) {
            ...
            try {
                ...
                showWorkspaceAlert(root, err.message || 'Unable to send message.', 'danger');
            } finally {
