                                        ...
                                        <?php if ($canPay): ?>
                                            <div class="mt-4">
                                                <button
                                                    ...
                                                    type="button"
                                                    id="pay-now-btn"
                                                    data-booking-id="<?= (int) $selectedBooking['id'] ?>"
                                                >
...
function cleanupWorkspace() {
    ...
    if (workspaceState.routingControl && workspaceState.detailMap) {
        try {
            workspaceState.detailMap.removeControl(workspaceState.routingControl);
        } catch (e) {}
...
function initSenderWorkspace() {
    ...
    if (bookingMapEl) {
        ...
        if (pickupLat?.value && pickupLng?.value) {
            updateFormMarker('pickup', parseFloat(pickupLat.value), parseFloat(pickupLng.value));
        }
        if (deliveryLat?.value && deliveryLng?.value) {
            updateFormMarker('delivery', parseFloat(deliveryLat.value), parseFloat(deliveryLng.value));
        }
    }
    ...
    if (
        ...
        window.clearActiveRoute = function () {
            clearRouteInternal();
            if (workspaceState.detailMap) {
                workspaceState.detailMap.flyTo(pickupCoords, 13);
            }
        };
        ...
        if (shouldSearchRiders) {
            ...
            async function updateRiders() {
                if (!shouldSearchRiders || selectedHasRider || ['matched', 'accepted', 'picked_up', 'in_transit', 'delivered', 'cancelled'].includes(String(selectedBookingStatus))) {
                    stopRiderSearch();
                    return;
                }
