...
header('X-Payments-Initialize-Version: 2026-03-25-01');
...
if ($secretKey === '') {
    ...
    echo json_encode([
        'status' => false,
        'message' => 'Paystack secret key is missing from env.php'
    ]);
    exit;
}
...
if ($appUrl === '' || !preg_match('#^https?://#i', $appUrl)) {
    ...
    echo json_encode([
        'status' => false,
        'message' => 'app_url is missing or invalid in env.php',
        'debug_app_url' => $appUrl
    ]);
    exit;
}
...
if (!is_array($data)) {
    ...
    echo json_encode([
        'status' => false,
        'message' => 'Invalid JSON returned by Paystack',
