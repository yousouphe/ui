            ...
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content bg-white text-dark border-0 shadow-lg">
                    ...
                    <div class="modal-body p-0">
                        <div id="leaflet-map-picker" style="height:420px;width:100%"></div>
                    </div>
                    <div class="modal-footer border-top">
                        <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal"><?= e(t('common.cancel')) ?></button>
...
function initSenderWorkspace() {
    ...
    function openMapPickerModal(initialLat, initialLng) {
        return new Promise((resolve) => {
            const modalEl = root.querySelector('#mapPickerModal');
            if (!modalEl || !window.bootstrap || !window.L) {
                resolve(null);
                return;
            }
            ...
            const onShown = function () {
                ...
                if (!mapPickerMap) {
                    ...
                    mapPickerMap.setView(center, initialLat ? 15 : 6);
    ...
    function attachAutocomplete(inputEl, suggestionsEl, latEl, lngEl, onSelected) {
            ...
            debounceTimer = setTimeout(async () => {
                ...
                try {
                    ...
                    renderSuggestions(suggestionsEl, items, (item) => {
                        ...
                        if (latEl) latEl.value = item.lat.toFixed(7);
                        if (lngEl) lngEl.value = item.lng.toFixed(7);
                        if (onSelected) onSelected(item.lat, item.lng);
                    });
                } catch (e) {
    ...
    if (bookingMapEl) {
        ...
        async function drawRoutePreview() {
            ...
            if (workspaceState.routingControl) {
                workspaceState.routingControl.remove();
                workspaceState.routingControl = null;
            }
            ...
            map.fitBounds(workspaceState.routingControl.getBounds(), { padding: [40, 40] });
        ...
        attachAutocomplete(pickupAddress, pickupSuggestions, pickupLat, pickupLng, (lat, lng) => updateFormMarker('pickup', lat, lng));
        attachAutocomplete(deliveryAddress, deliverySuggestions, deliveryLat, deliveryLng, (lat, lng) => updateFormMarker('delivery', lat, lng));
    ...
    if (changeDeliveryForm) {
        ...
        attachAutocomplete(editDeliveryAddress, editDeliverySuggestions, editDeliveryLat, editDeliveryLng, null);
        ...
        editDeliveryMapPickLink?.addEventListener('click', async function (e) {
            ...
            const picked = await openMapPickerModal(initialLat, initialLng);
            if (!picked) return;
